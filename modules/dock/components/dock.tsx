...
function DockButton({ icon: Glyph, label, active, playing, onClick }: { icon: Glyph; label: string; active?: boolean; playing?: boolean; onClick: () => void }) {
  return <button aria-current={active ? "page" : undefined} aria-label={label} aria-pressed={active} className={cn("relative grid size-11 place-items-center rounded-control border transition-colors duration-200 motion-reduce:transition-none", active ? "border-sage-500 bg-sage-500 text-sage-1000" : "border-sage-700 bg-sage-1000 text-sage-100 hover:border-sage-500 hover:bg-sage-800")} onClick={onClick} title={label} type="button"><Glyph className="size-5" weight={active ? "Bold" : "Linear"} />{playing ? <span className="absolute right-2 top-2 size-1.5 rounded-full bg-[#C97952]" /> : null}</button>;
}
