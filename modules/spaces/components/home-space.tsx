...
export function HomeSpace() {
  ...
  return <section className="relative grid min-h-dvh place-items-center overflow-hidden bg-base-200 px-5 py-24 text-center"><SpaceBackground /><div className="relative max-w-3xl"><p className="text-sm font-semibold uppercase tracking-[0.2em] text-sage-700">{date}</p><h1 className="mt-5 text-[clamp(2.75rem,8vw,6.75rem)] font-bold leading-[0.94] tracking-[-0.045em] text-sage-1000">{mounted ? t(`headline.${getTimeBand(now)}`, { name }) : ""}</h1><p className="khulwa-numeric mt-8 text-[clamp(3rem,7vw,6rem)] font-semibold tracking-[-0.055em] text-sage-800">{time}</p><p className="mt-5 text-base text-sage-700">{t("subtitle")}</p></div></section>;
}
