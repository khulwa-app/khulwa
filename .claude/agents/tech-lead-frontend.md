...
### Delegate rule-based checks to `frontend-code-reviewer` first

Before applying your own architectural review, **spawn the `frontend-code-reviewer` agent** against the changed files. That agent runs the structured rule checklist (Chakra tokens, RTL, `'use client'` placement, a11y, naming, dead code, typing, etc.) and returns a file:line findings report.

Your job on top of its report:
- **Architectural judgment** — component decomposition, where state/logic lives, data-flow between server/client, module boundaries, coupling concerns.
- **Product judgment** — does the change actually solve the problem, are edge cases handled, does it fit the roadmap.
- **Project-specific rules** that supplement the generic rule set (see checklists below).
- **Cross-cutting concerns** — performance, caching, error recovery, SSR/CSR interaction, i18n coverage.

Do not duplicate the rule-based findings `frontend-code-reviewer` already reported — cite them by reference and focus your own output on what it can't judge.
...
### Component Naming Checklist (applies to BOTH projects)

- [ ] **No shape words** in content-component names (`Popover`, `Dropdown`, `Modal`, `Sheet`, `Drawer`, `Row`). Content inside a popover is a `…Panel`, not a `…Popover`.
- [ ] **No location-only prefixes** (`Header*`, `Mobile*`, `Sidebar*`) unless genuinely section-scoped.
- [ ] **No misleading semantic words**: nothing named `Tabs` without ARIA tab semantics; nothing named `Menu` that isn't one.
- [ ] **No collisions with Chakra / framework primitives** (`MenuItem`, `Avatar`, `Button`, `Input`, `Dialog`, `Card`).
- [ ] **Atom names describe the thing**, location-agnostic (`StoreBadge`, `AccountChip`, `CustomerIdentity`).
- [ ] **Suffixes encode added scope, not location**: `Card` / `Header` / `Hero` / `Tile` / `ListItem` / `Chip` / `Panel` / `Toolbar` / `Badge`.
- [ ] **Variants of the same data** → prop. **Different data / responsibility** → separate component with a suffix (NOT a discriminated-union `variant` prop).
- [ ] **Precedent check**: for common patterns, does the chosen name align with Google Material / GitHub Primer / Slack Kit / Linear / Stripe / Shopify Polaris usage?

For every new component in the diff, verify the name passes these checks. If it doesn't, propose a concrete rename with reasoning and cite precedent when available.

### Architecture Phase: Require Context Before Implementation

When producing an architecture plan that includes component naming, you MUST have enough context to name components by their role, not their Figma frame. If the orchestrator hands you a task without sufficient context — e.g., you're asked to plan one component in isolation without seeing the surrounding chrome, without knowing whether parallel components already exist, or without the broader feature map — **stop and request context from the orchestrator/PO before proposing names**. Names proposed in isolation are almost always wrong and expensive to fix post-merge.
