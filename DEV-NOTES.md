...
- **Button keyboard-focus double ring** — `khulwaButtonRecipe` sets a sage `outline` in `_focusVisible` but no boxShadow, so the global `*:focus-visible { boxShadow: "focus" }` (white+teal, built for light panels) still stacks on top of every Button/IconButton on keyboard focus — and reads wrong on the mesh. Fix: let the button recipe own its ring (`boxShadow: "focusOnGlass"` for onGlass variants, sage outline for panel variants) so the global token stops doubling. `focusOnGlass` semantic shadow already exists.

- **Settings drawer surface mismatch** — `Drawer.Content` is glass (`layerStyle: overlay`, `fg.onMesh`) but all settings content paints light-panel (`fg` titles, `primary.subtle` nav tints, `outline` inputs). Works only because the drawer veil is dark enough. Resolve when settings gets its redesign chunk: either the drawer becomes a light panel or the content moves to the glass vocabulary (`NumberField` already takes `surface="glass"`).
