...
export function FocusSpace() {
  ...
  useEffect(() => { if (timer.completionCount <= loggedCompletion.current) return; loggedCompletion.current = timer.completionCount; if (timer.lastCompletedPhase !== PomodoroPhase.Focus) return; const endedAt = new Date(); logSession({ durationSeconds: timer.focusMinutes * 60, startedAt: new Date(endedAt.getTime() - timer.focusMinutes * 60_000).toISOString(), endedAt: endedAt.toISOString() }); }, [logSession, timer.completionCount, timer.focusMinutes, timer.lastCompletedPhase]);

  if (!hydrated) return <section className="min-h-dvh bg-sage-1000" />;
  return <section className="relative grid min-h-dvh place-items-center overflow-hidden bg-sage-1000 px-5 py-24"><SpaceBackground tone="night" /><div className="relative grid justify-items-center gap-7 text-center"><PhaseTabs currentRound={timer.currentRound} onPhaseChange={timer.setPhase} phase={timer.phase} tone="dark" totalRounds={timer.totalRounds} /><p className="khulwa-numeric text-[clamp(5rem,16vw,11rem)] font-bold leading-none tracking-normal text-sage-100">{formatPomodoro(timer.minutes, timer.seconds)}</p><DoingNowCaption tone="dark" /><div className="flex items-center gap-4"><button aria-label={t("actions.reset")} className="grid size-12 place-items-center rounded-full border border-sage-700 bg-sage-900 text-sage-100 hover:border-sage-500 hover:bg-sage-800" onClick={timer.reset} type="button"><ControlIcon kind="reset" /></button><button aria-label={timer.isRunning ? t("actions.pause") : timer.hasStarted ? t("actions.resume") : t("actions.begin")} className="grid size-16 place-items-center rounded-full bg-sage-500 text-sage-100 shadow-none transition-colors hover:bg-sage-400" onClick={timer.isRunning ? timer.pause : timer.start} type="button"><ControlIcon kind={timer.isRunning ? "pause" : "play"} /></button><button aria-label={t("actions.skip")} className="grid size-12 place-items-center rounded-full border border-sage-700 bg-sage-900 text-sage-100 hover:border-sage-500 hover:bg-sage-800" onClick={timer.skip} type="button"><ControlIcon kind="skip" /></button></div></div></section>;
}
